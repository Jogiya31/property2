document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (typeof hydrateForm2ForEdit === "function") {
    hydrateForm2ForEdit();
  }
...
function initForm2() {
  ...
  const acquisitionGift = document.getElementById("acquisition_gift_form2");
  const date_acq_dis = document.getElementById("date_acq_dis");

  const form2Inparts = document.getElementById("form2_inparts");
  const form2Full = document.getElementById("form2_full");
  ...
  window.__appendPropertyBlockForm2 = appendPropertyBlockForHydration;
  ...
  function toggleGift() {
    ...
    if (mode === "Gift" && acquiredDisposed?.value === "acquisition") {
  ...
  purpose?.addEventListener("change", function () {
    if (this.value === "Sanction for transaction") {
      safeShow(form2Inparts);
      safeHide(form2Full);
    } else {
      safeShow(form2Full);
      safeHide(form2Inparts);
    }
  });
  ...
  form.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".addRow_applicant")) {
      ...
      if (!isHydratingEdit) {
        // Check if all inputs/selects in last row are filled
        ...
        inputs.forEach((el) => {
          if (!el.value || el.value.trim() === "") {
            isValid = false;
            el.classList.add("error"); // optional highlight
          } else {
      ...
      // Clone only if valid (or edit hydration)
      ...
      ========================= */
        ...
        if (sourceBlock && isVisible(sourceBlock)) {
          ...
          if (price !== sourceTotal) {
            return showError(
              firstSource,
              `Source total ₹${sourceTotal} must equal property price ₹${price}`,
            );
          }
        }
...
function createPayload() {
  ...
  document.querySelectorAll(".property-block").forEach((block, pIndex) => {
    /* ================= DISPOSAL FILE ================= */
    ...
    if (disposalFile) {
      disposalFileKey = `disposal_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
      files.push({
        key: disposalFileKey,
        file: disposalFile,
      });
    ...
    const property = {
      ...
      nature_dealing_party:
        block.querySelector('[name="nature_dealing_party[]"]')?.value || "",
...
/** Programmatic `value = …` does not fire `input`; normalization and price-in-words only run on `input`. */
function dispatchAmountFieldInputSync(block) {
  if (!block) return;
  const price = block.querySelector('[name="property_price[]"]');
  if (price) {
    price.dispatchEvent(new Event("input", { bubbles: true }));
  }
  block.querySelectorAll('[name="source_amount[]"]').forEach((el) => {
...
function fillPropertyBlock(block, property) {
    ...
    property.sources.forEach((source, idx) => {
      ...
      if (source?.attachment?.download_url && sourceFile) {
        ...
        if (linkWrap) {
          ...
          linkWrap.innerHTML = `<a href="../api/view_attachement_file.php?file_key=${key}" target="_blank" rel="noopener"> ${name}</a>`;
  ...
  dispatchAmountFieldInputSync(block);
}

function hydrateForm2ForEdit() {
  const form = document.getElementById("form2");
  const data = window.__editFormData;
  const ft = String(data?.form_type || "").toLowerCase();
  if (!form || !data || ft !== "movable") return;
  ...
  if (propertyList.length) {
    ...
    try {
      ...
      while (
        ...
        if (typeof window.__appendPropertyBlockForm2 === "function") {
          window.__appendPropertyBlockForm2();
        }
...
function submitForm() {
  ...
  openConfirmModal("Are you sure you want to submit the form?", async () => {
    ...
    // main fields
    formData.append("form_type", "movable");
